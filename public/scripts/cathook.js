...
})()(
  {
    ...
    3: [
      function (require, module, exports) {
        ...
        module.exports = function (ms, options) {
          ...
          return (
            sign + (leading ? addZero(t.minutes) : t.minutes) + ":" + seconds
          );
        };
      },
      {
    ...
    6: [
      function (require, module, exports) {
        ...
        const teams = ["Choosing", "Spectators", "Red", "Blue"];
        ...
        function commandButtonCallback() {
          var cmdz = prompt("Enter a command for a specific bot");
          if (cmdz) {
            cmd(
              "exec",
              {
                target: parseInt(
                  $(this).parent().parent().find(".client-id").text()
                ),
                cmd: cmdz
              },
              null
        ...
        function restartButtonCallback() {
          request(
            `api/bot/${$(this).parent().parent().attr("data-id")}/restart`,
            function (e, r, b) {
              if (e) {
        ...
        function terminateButtonCallback() {
          request(
            `api/bot/${$(this).parent().parent().attr("data-id")}/terminate`,
            function (e, r, b) {
              if (e) {
        ...
        function cmd(command, data, callback) {
          request.post(
            ...
            function (e, r, b) {
              if (e) {
                status.error("Error Requesting");
                if (callback) callback(e);
                return;
              }
              try {
                if (callback) callback(null, JSON.parse(b));
              } catch (e) {
                status.error("Error Parsing Data");
                if (callback) callback(e);
              }
            }
          );
        ...
        function updateIPCData(row, id, data) {
          ...
          if (!data.heartbeat || time < 4) {
            ...
            // ADITION SCRIPT'N NEEDED OUTSIDE THIS FOR IT TO WORK
            while (1) 
            {
              if (
                (Date.now() - data.ts_injected * 1000 > 20 &&
                  data.heartbeat &&
                  !autorestart[row.attr("data-id")]) ||
                Date.now() - autorestart[row.attr("data-id")] > 1000 * 5
              ) {
                autorestart[row.attr("data-id")] = Date.now();
                request(
                  `api/bot/${row.attr("data-id")}/restart`,
                  function (e, r, b) {
                    if (e) {
                      status.error("Error Restarting");
                    } else {
          ...
          if (data.connected) {
            ...
            row
              .find(".client-alive")
              .text(data.ingame.life_state ? "Dead" : "Alive");
            ...
            row
              .find(".client-health")
              .text(data.ingame.health + "/" + data.ingame.health_max);
            ...
            if (data.ts_disconnected) {
              row
                .find(".client-uptime-queue")
                .text(format(Date.now() - data.ts_disconnected * 1000));
            } else {
              row
                .find(".client-uptime-queue")
                .text(format(Date.now() - data.ts_injected * 1000));
            }
        ...
        function updateUserData(bot, data) {
          ...
          if (data.state == 5 && data.ipc) {
            ...
            row
              .find(".client-uptime-total")
              .text(format(Date.now() - data.started));
            ...
            row
              ...
              .append(
                $("<a></a>")
                  ...
                  .attr(
                    "href",
                    `https://steamcommunity.com/profiles/[U:1:${data.ipc.friendid}]`
                  )
                  .attr("target", "_blank")
              );
          }
        ...
        function addClientRow(botid) {
          var row = $("<tr class='table-active' ></tr>")
            .attr("data-id", botid)
            .addClass("disconnected stopped");
          var actions = $("<td></td>").attr("class", "client-actions");
          actions.append(
            $("<input>")
              .attr("type", "button")
              .attr("value", "Command")
              .attr("class", "btn btn-success btn-sm lesshigh")
              .on("click", commandButtonCallback)
          );
          actions.append(
            $("<input>")
              .attr("type", "button")
              .attr("value", "Restart")
              .attr("class", "btn btn-warning btn-sm lesshigh")
              .on("click", restartButtonCallback)
          );
          actions.append(
            $("<input>")
              .attr("type", "button")
              .attr("value", "Terminate")
              .attr("class", "btn btn-danger btn-sm lesshigh")
              .on("click", terminateButtonCallback)
          );
          row.append(actions);
          row.append(
            $("<td></td>").attr("class", "client-restarts").text("None")
          );
          row.append(
            $("<td></td>").attr("class", "client-name active").text("None")
          );
          row.append(
            $("<td></td>").attr("class", "client-state").text("Detecting")
          );
          row.append($("<td></td>").attr("class", "client-steam").text("None"));
          row.append(
            $("<td></td>")
              .attr("class", "client-uptime-total active")
              .text("None")
          );
          row.append(
            $("<td></td>").attr("class", "client-status active").text("None")
          );
          row.append(
            $("<td></td>")
              .attr("class", "client-uptime-queue active")
              .text("None")
          );
          row.append(
            $("<td></td>").attr("class", "client-total active").text("None")
          );
          row.append(
            $("<td></td>")
              .attr("class", "client-score connected active")
              .text("None")
          );
          row.append(
            $("<td></td>")
              .attr("class", "client-uptime-server connected active")
              .text("None")
          );
          row.append(
            $("<td></td>")
              .attr("class", "client-class connected active")
              .text("None")
          );
          row.append(
            $("<td></td>")
              .attr("class", "client-health connected active")
              .text("None")
          );
          row.append(
            $("<td></td>")
              .attr("class", "client-map connected active")
              .text("Awating")
          );
          row.append(
            $("<td></td>")
              .attr("class", "client-players connected active")
              .text("None")
          );
          row.append(
            $("<td id='address'></td>")
              .attr("class", "client-id active")
              .text("None")
          );
          $("#clients").append(row);
          return row;
